...
documentation, configuration files and commit messages.

This is not only a style preference.
The `typos` hook runs from pre-commit over every file, comments included,
and it reads French as misspelled English — if two words sound the same it will,
correct the french word to the english word and the commit is refused.
...
## Remote RPMs in build.sh

Never hand a URL to `dnf install` in [build.sh](./build_files/build.sh).
Download the rpm to `/tmp` with `curl`, then install the local file.

Given a URL, dnf keeps the rpm under `/var/cache/libdnf5/@commandline-*`,
even with `keepcache=0`.
That directory lives in the `--mount=type=cache,dst=/var/cache` of the
`Containerfile`, where the rpm is left zero-filled between local builds.
The next download is appended to that copy instead of replacing it,
and the build fails with a misleading `... : not a rpm`.
CI always starts cold, so only local builds break, from the second one on.

A cache poisoned before this rule can be cleared with
`rm -rf /var/tmp/buildah-cache-$(id -u)`.
